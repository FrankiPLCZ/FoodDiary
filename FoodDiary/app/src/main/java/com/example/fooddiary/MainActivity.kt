        ...
        if(cur.moveToFirst()) {
            if(!(cur.getString(1).isNullOrEmpty())){
                ...
                val mx = if(cur.getString(6)=="Male")
                            ((cur.getDouble(2) *10 + cur.getDouble(4)*6.25 -
                                    cur.getDouble(3) * 5 + 5)*cur.getDouble(5)).toInt()
                        else
                            ((cur.getDouble(2) *10 + cur.getDouble(4)*6.25 -
                                    cur.getDouble(3) * 5 -161)*cur.getDouble(5)).toInt()
                b.Kcal.max = mx
            ...
            val cur2 = db2.query(TableInfo2.TABLE_NAME, null, null,
                    null, null, null, null)
            if(!cur2.moveToFirst()){
                for(i in 0..7){
